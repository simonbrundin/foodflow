...
/**
 * Zitadel OAuth 2.0 handler using PKCE
 *
 * This endpoint handles the OAuth flow:
 * 1. Initial request: redirects to Zitadel login
 * 2. Callback: exchanges code for tokens and creates session
 */
  ...
  try {
    ...
    await setUserSession(event, {
      user: {
        ...
        name: userInfo.name || `${userInfo.given_name || ''} ${userInfo.family_name || ''}`.trim() || userInfo.email || 'User',
